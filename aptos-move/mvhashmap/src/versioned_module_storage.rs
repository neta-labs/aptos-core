    ...
    VersionedModuleStorage<K>
{
    ...
    pub fn get(&self, key: &K, txn_idx: TxnIndex) -> ModuleStorageRead {
        if CrossBlockModuleCache::is_invalidated() {
            self.get_impl(key, txn_idx).unwrap_or(ModuleStorageRead::DoesNotExist)
        } else {
            CrossBlockModuleCache::get_from_cross_block_module_cache(key.as_state_key())
                .map(|e| ModuleStorageRead::Versioned(Err(StorageVersion), e))
    ...
    {
        if CrossBlockModuleCache::is_invalidated() {
            if let Some(result) = self.get_impl(key, txn_idx) {
                return Ok(result);
            }
