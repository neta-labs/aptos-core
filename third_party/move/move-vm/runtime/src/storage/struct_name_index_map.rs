...
#[derive(Clone)]
struct IndexMap<T: Clone + Ord + Hash> {
    forward_map: DashMap<T, usize>,
    backward_map: DashMap<usize, Arc<T>>,
}
...
pub(crate) struct StructNameIndexMap(IndexMap<StructIdentifier>);
...
impl StructNameIndexMap {
    ...
    pub(crate) fn flush(&self) {
        self.0.forward_map.clear();
        self.0.backward_map.clear();
    }
    ...
    pub(crate) fn struct_name_to_idx(&self, struct_name: StructIdentifier) -> StructNameIndex {
        ...
        if let Some(idx) = self.0.forward_map.get(&struct_name) {
            return StructNameIndex(*idx);
        }

        match self.0.forward_map.entry(struct_name.clone()) {
            Entry::Occupied(e) => {
                StructNameIndex(*e.get())
            },
            Entry::Vacant(e) => {
                let idx = self.0.backward_map.len();
                self.0.backward_map.insert(idx, Arc::new(struct_name));
                e.insert(idx);
                StructNameIndex(idx)
            },
        }
    ...
    pub(crate) fn idx_to_struct_name_ref(&self, idx: StructNameIndex) -> Arc<StructIdentifier> {
        self.0.backward_map.get(&idx.0).unwrap().clone()
    }
    ...
    pub(crate) fn idx_to_struct_name(&self, idx: StructNameIndex) -> StructIdentifier {
        self.0.backward_map.get(&idx.0).unwrap().as_ref().clone()
    }
    ...
    pub(crate) fn idx_to_struct_tag(
        ...
        let a = self.0.backward_map.get(&idx.0).unwrap();
        let struct_name = a.as_ref();
        StructTag {
